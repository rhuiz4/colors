...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
  float xt, yt, zt, xb, yb, zbo, xm, ym, zm, x0, x1, d0x, d1x, y0, z0, z1, d0z, d1z;
  int r, g, b;
  color c;

  //struct matrix *pts = new_matrix(3,3);      //index   0:top   1:mid    2:bot
  int top, mid, bot;
  //printf("%d: %.2f, %.2f, %.2f\n", i, points->m[0][i], points->m[1][i], points->m[2][i]);
  //printf("%d: %.2f, %.2f, %.2f\n", i+1, points->m[0][i+1], points->m[1][i+1], points->m[2][i+1]);
  //printf("%d: %.2f, %.2f, %.2f\n", i+2, points->m[0][i+2], points->m[1][i+2], points->m[2][i+2]);
  if(points->m[1][i] > points->m[1][i+1]){     //if p1 > p2
    top = i;
    }
  else{                                                
    top = i+1;
  }
  if(points->m[1][top] < points->m[1][i+2]){       //if p3 is max
    if(top == i){
      top = i+2;
      mid = i;
      bot = i+1;
  ...
  if(top == i){                                    //if p1 is max
    if(points->m[1][i+1] > points->m[1][i+2]){ //if p2 > p3
      mid = i+1;
      bot = i+2;
    }
    else{
...
/*======== void add_polygon() ==========
  xsInputs:   struct matrix *surfaces
  double x0
  double y0
  double z0
  double x1
